    ...
    def like(self, request, pk):
        ...
        if request.method == 'POST':
            if user.likes.filter(post=post).exists():
                return Response('This post has already liked!', status=200)
    ...
    def favorite(self, request, pk):
        ...
        if request.method == 'POST':
            if user.favorites.filter(post = post).exists():
                return Response('This post already added in favorite!', status=400)
            ...
            if favorite.exists():
                favorite.delete()
                return Response('Post successfully deleted by you!')
        else:
